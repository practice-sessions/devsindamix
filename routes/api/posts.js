...
//@route   GET api/posts/:id        <Single posts>
//@desc    Get post by id
//@access  Public
router.get('/:id', (req, res) => {
  Post.findById(req.params.id).then(post => res.json(post))
    .catch(err => res.status(404).json({ nopostfound: 'No post found for this user' }));
});
...
router.post('/like/:id', passport.authenticate('jwt', { session: false }),
  (req, res) => {
    Profile.findOne({ user: req.user.id }).then(profile => {
      Post.findById(req.params.id).then(post => {
        // Check if user already liked post in (likes) array
...
//@access  Private - reflects protected route status
router.post('/comment/:id', passport.authenticate
...
//@access  Private - reflects protected route status
router.delete('/comment/:id/:comment_id', passport.authenticate
  ('jwt', { session: false }), (req, res) => {
    Post.findById(req.params.id).then(post => {
